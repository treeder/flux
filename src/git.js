...
export function createPullRequest(branchName, title, cwd = process.cwd(), issueNumber = null) {
  try {
    ...
    let body = `Automated PR generated by flux agentic VCS for intent: ${title.replace(/"/g, '\\"')}`
    if (issueNumber) {
      body += `\n\nCloses #${issueNumber}`
    }
    const prUrl = run(`gh pr create --title "${title.replace(/"/g, '\\"')}" --body "${body}"`, cwd)

...
export function createPullRequest(branchName, title) {
  try {
    console.log(`Pushing branch ${branchName} to origin...`)
    run(`git push -u origin ${branchName}`)
    
    try {
      const existingPrs = run(`gh pr list --head ${branchName} --json url`);
    ...
    run(`gh pr create --title "${title.replace(/"/g, '\\"')}" --body "Automated PR generated by flux agentic VCS for intent: ${title}"`)
